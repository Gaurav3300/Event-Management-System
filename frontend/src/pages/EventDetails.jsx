...
export default function EventDetails() {
  ...
  async function register() {
    try {
      ...
      const checkRes = await axios.get(`/api/registrations/me`);
      const isRegistered = checkRes.data.registrations?.some(reg => reg.event?._id === id);
       ...
       <img src={event.posterUrl || '/placeholder.svg'} className="w-full h-64 object-cover rounded-xl" />
        <div>
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button 
              className="px-4 py-2 rounded-lg font-medium border border-slate-200 dark:border-slate-700 
